...
int WebRtcVad_Process(VadInst *handle, int fs, const int16_t *audio_frame,
  ...
  if (handle == NULL) {
    std::cout << "handle null error" << std::endl;
    return -1;
  }

  if (self->init_flag != kInitCheck) {
    std::cout << "has not init error" << std::endl;
    return -1;
  }
  if (audio_frame == NULL) {
    std::cout << "audio frame null error" << std::endl;
    return -1;
  }
  if (WebRtcVad_ValidRateAndFrameLength(fs, frame_length) != 0) {
    std::cout << "invalid rate and frame length error" << std::endl;
    return -1;
  }
  ...
  if (vad == -1) {
    std::cout << "vad check failed" <<std::endl;
  }
  return vad > 0 ? 1 : vad;
}
